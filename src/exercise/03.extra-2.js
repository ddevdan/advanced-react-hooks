...
import {
  fetchPokemon, PokemonForm, PokemonDataView, PokemonInfoFallback, PokemonErrorBoundary,
} from '../pokemon'
...
function PokemonInfo({ pokemonName }) {
  ...
  React.useEffect(() => {
    if (!pokemonName) {
      ...
      run(fetchPokemon(pokemonName).then(pokemonData => {
...
function PreviousPokemon({ onSelect }) {
  ...
  return (<div>
    ...
    <ul style={{ listStyle: 'none', paddingLeft: 0 }}>
      {Object.keys(cache).map(pokemonName => (<li key={pokemonName} style={{ margin: '4px auto' }}>
        <button
          style={{ width: '100%' }}
